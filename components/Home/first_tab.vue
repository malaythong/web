...
export default {
  ...
  created() {
    ...
    this.$root.$on('search-updated', this.performSearch);
  },
  methods: {
    checkRole(){
      if (this.localeRole === 'user') {
    ...
    async performSearch(query) {
      try {
        console.log('search check', this.query)
        const res = await this.$apollo.query({
          query: gql`
            query getForumAll($topic: String!) {
              forum(where: { topic: { _ilike: $topic } }) {
                updated_at
                topic
                id
                tag_id
                ...
                ratings(where: { user_id: 1 }, limit: 1) {
                  forum_id
                  id
                  score
                  user_id
                }
                ratings_aggregate(where: { user_id: 1 }) {
                  aggregate {
                    count(columns: id)
                  }
                }
