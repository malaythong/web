    ...
    <div>
        <v-row justify="center">
            <v-col class=" mt-10" cols="12" sm="9">
                <v-text-field label="Search" placeholder="ຊື່ແທັກ" outlined hide-details solo dense
                    class="custom-text-field mb-12" append-icon="mdi-magnify"></v-text-field>
                <v-card elevation="0">
                    ...
                    <v-list >
                        <v-list-item-group>
                            <template>
                                <v-list-item v-for="(item, index) in getUserTagAll" :key="index">
                                    <template>
                                        <v-list-item-content>
                                            <v-row>
                                        ...
                                        <v-list-item-action>
                                            <v-btn depressed @click="DeleteTag(item.tag.id)" color="error">ຍົກເລີກຕິດຕາມ
                                            </v-btn>
                                        </v-list-item-action>
                                    </template>
                                </v-list-item>
                            ...
                            <template>
                                <v-list-item v-for="(item, index) in filteredTags" :key="index">
                                    <template>
                                        ...
                                        <v-list-item-action>
                                            <v-btn depressed @click="InsertTag(item.id)" color="primary">ຕິດຕາມ
                                            </v-btn>
                                        </v-list-item-action>
                                    </template>
                                </v-list-item>
...
export default {
   ...
   // this.retrievedData = localStorage.getItem("userData");
    ...
    computed: {
    filteredTags() {
        return this.getTagAll ? this.getTagAll.filter(tagItem => !this.tagIds.includes(tagItem.id)) : [];
    },
    tagIds() {
    ...
    DeleteTag(id) {
     ...
     console.log(error)
      ...
      console.log("run test tag")
           await this.$apollo.query({
                query: require('~/gql/queries/home/get_al_tag_by_user.gql')
                  .MyQuery,
                fetchPolicy: 'no-cache',
                variables: { 
         userId:this.localeId
